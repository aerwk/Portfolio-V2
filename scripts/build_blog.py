...
# v3 cutover (2026-09-24): blog/posts/ and blog/index.html are now v3 output,
# regenerated by `scripts/build_dev_preview.py --prod`. This script's target
# is the retired v2 site, moved to blog/v2/ -- it must never write into the
# live v3 paths. See CLAUDE.md's stub-journal warning before running this
# against real journals regardless.
POSTS_DIR = os.path.join(REPO, "blog", "v2", "posts")
INDEX = os.path.join(REPO, "blog", "v2", "index.html")
...
def main():
    ...
    # v3 cutover (2026-09-24): build_home() is NOT called any more.
    # home/index.html is now v3 output -- its latest-posts grid is
    # regenerated by scripts/build_dev_preview.py --prod (from
    # blog/v2/posts/ via posts_json_html/blog_list_view_html), not by this
    # script. build_home() (below) is left in place, unused, rather than
    # deleted -- its <ul class="posts"> patch logic only matches the old v2
    # home markup and would corrupt v3's home/index.html if ever called
    # against it.
