...
With the drive properly named, set up two Samba shares on top of it. `homeshare` is guest-accessible, read/write, available to anything on the network — a general-purpose drop point. `[user]` is private, requires a Samba password, for personal files. Both share blocks added to `/etc/samba/smb.conf`, Samba restarted. The shares are reachable locally and through Tailscale using `smb://x.x.149.70/homeshare` (or `/[user]`).
