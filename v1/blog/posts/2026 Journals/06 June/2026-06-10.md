...
Tailnet is now clean — OptiPlex (`n5ubuntu`, `x.x.149.70`) and MacBook Pro (`erics-macbook-pro`, `x.x.153.21`) both confirmed online. The Samba share on the server is accessible from the MacBook via `smb://x.x.149.70/shared` through Tailscale when off the local network.
...
Swapped the OptiPlex from the onboard 1GbE NIC (`eno1`) to a 2.5GbE USB-C adapter (`enx74****5c7c`). IP held at `[redacted-ip]` via UniFi fixed DHCP reservation. The old NIC stays configured in Netplan but unused — available for future use if needed (Proxmox management network, etc.). The 1GbE ceiling would've become a real bottleneck the moment a NAS or Kubernetes cluster enters the picture. Better removed now.
